    ...
    public BITalinoFrame[] read(final int[] analogChannels, final int totalBytes,
        ...
        try {
            ...
            while (sampleCounter < numberOfSamples) {
                ...
                // Added by sveinpg
                currentSeq = f.getSequence();
                if (currentSeq != (lastSeq+1)%16) {
                    Log.d("Sequence check", "missing packet " + lastSeq + ", " + currentSeq);
                }
                Log.d("Sequence check", "sequences " + lastSeq + ", " + currentSeq);
                lastSeq = currentSeq;
                ...
                if (f.getSequence() == -1) {
                    ...
                    // throw exception
                    Log.d("Bitalino read", "Missed a sequence. Are we too far from BITalino? Retrying..");
